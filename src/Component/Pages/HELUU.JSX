...
import { users } from '../Pages/Userdb';
...
const UserDashboard = () => {
  ...
  const enrolledCoursesWithProgress = currentUser.enrolledCourseIds
    .map((courseId) => {
      ...
      if (!course) {
        console.warn(`Course data for ID: ${courseId} not found.`);
        return { id: courseId, missing: true, isFree: false, title: `Unknown Course (${courseId})` };
      }
  ...
  return (
    <div className="p-6 bg-gray-100 min-h-screen font-sans">
      ...
      {missingCourses.length > 0 && (
        <div className="mb-10">
          <h4 className="text-2xl font-bold text-red-600 mb-6 border-b-2 border-red-300 pb-2">Missing Course Data</h4>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {missingCourses.map((course) => (
              <div
                key={course.id}
                className="bg-white rounded-xl shadow-lg p-6 border-l-4 border-red-500 text-red-700"
              >
                <p className="font-semibold text-lg">Course Not Found:</p>
                <p className="text-md italic">{course.id}</p>
                <p className="text-sm mt-2">
                  This course ID is not found in course data.
                </p>
              </div>
            ))}
          </div>
