...
// Add currentUrl prop to display the already uploaded video
const CloudinaryVideoUploader = ({ onUploadSuccess, currentUrl }) => {
...
    const resourceType = selectedFile.type.startsWith('video/') ? 'video' : 'raw';
    
    const formData = new FormData();
    formData.append('file', selectedFile);
    formData.append('upload_preset', UPLOAD_PRESET);
    formData.append('folder', 'Paidcourse_media');
...
      {uploadedDisplayUrl ? (
        <div>
          <p className="text-sm text-gray-700 mb-2">Video uploaded:</p>
...
          </a>
          {/* Optionally display a video player */}
          <video controls src={uploadedDisplayUrl} className="max-w-full h-auto mt-2 mx-auto"></video>
          <button
            type="button"
            onClick={() => {
...
          {file && uploading ? `Uploading ${file.name}...` : 'Click to select video/audio'}
          <input
...
      {!uploading && file && !error && uploadedDisplayUrl && <p className="text-green-600 text-sm mt-2">Upload complete!</p>}
    </div>
  );
};
